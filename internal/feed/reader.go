...
func (r Reader) ReadMany(ctx context.Context, sources ...string) ReadResult {
	...
	for result := range results {
		if result.err != nil {
			log.WithField("feed", result.source).WithError(result.err).Error("Failed to parse feed")
		...
		total += result.count
		log.WithFields(log.Fields{"feed": result.source, "nets": result.count}).Info("Feed parsed")
	}
