...
- Release tags are `v*`; do not invent a parallel tag scheme without updating
  the workflows, README, and release docs together.
...
- Container publishing is limited to pushes to `main` and trusted `v*` tags.
- Release publishing is limited to trusted `v*` tags.
- Signed APT repository publishing is limited to trusted `v*` tags.
